...
void Initialise()
{
	ConsoleClearScreen(0x0F); 	//Clear the screen with cyan	
	ConsoleSetColour(0x0F);		//Set console to black text, grey back
	
	//Write display message and center each line
	//15 is assumed because I counted them. strlen should be used really
	ConsoleGetXY(&cursorX, &cursorY);
	ConsoleGotoXY(((ConsoleGetWidth()) / 2) - 15, cursorY + 1); //+1 for top room
	ConsoleWriteString("\\||||||||||||||||||||||||||||/\r\n");
	
	ConsoleGetXY(&cursorX, &cursorY);
	ConsoleGotoXY(((ConsoleGetWidth()) / 2) - 15, cursorY);	
	ConsoleWriteString("-  Welcome to RichOS Kernel  -\r\n");
	
	ConsoleGetXY(&cursorX, &cursorY);
	ConsoleGotoXY(((ConsoleGetWidth()) / 2) - 15, cursorY);
	ConsoleWriteString("/||||||||||||||||||||||||||||\\\r\n\n");
	
	ConsoleWriteString("Kernel size is ");
