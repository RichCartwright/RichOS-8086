...
%include "bpb.asm"					; We include a copy of the same BIOS parameter block that is used in stage 1
...
; Output:
;   absoluteSector = (LBA address / sectors per track) + 1   (We add one since the sectors start from 0)
;   absoluteHead   = (LBA address / sectors per track) MOD number of heads
;   absoluteTrack  = LBA address / (sectors per track * number of heads)
